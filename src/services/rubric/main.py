...
def score(req: ScoreRequest):
    ...
    if "citation" in req.answer.lower():
        acc = min(5, acc + 1)

    if req.prev_answer:
        sim = textdistance.jaccard.similarity(
            set(req.answer.lower().split()), set(req.prev_answer.lower().split())
        )
    ...
    scores = RubricScores(
        accuracy=acc, depth=depth, originality=originality, integrity=integrity
    )
