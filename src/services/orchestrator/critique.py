...
def critique_text(prompt: str, answer: str, rubric: RubricScores) -> str:
    ...
    if rubric.depth < 5:
        tips.append(
            "- Add 2–3 concrete examples or a step-by-step to deepen explanation."
        )
    if rubric.originality < 5:
        tips.append(
            "- Rephrase repeated ideas and add a novel analogy to increase originality."
        )
    if rubric.integrity < 5:
        tips.append(
            "- Remove speculative/unsafe advice; keep to verifiable statements."
        )
