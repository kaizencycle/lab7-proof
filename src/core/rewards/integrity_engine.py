...
def score_novelty(
    payload: dict[str, Any], reference_data: list[dict] | None = None
) -> float:
    """
    ...
    content = (
        f"{payload.get('human_statement', '')} {payload.get('agent_statement', '')}"
    )
...
def score_entropy(payload: dict[str, Any]) -> float:
    ...
    for pos, neg in contradiction_words:
        if (pos in human_lower and neg in agent_lower) or (
            pos in agent_lower and neg in human_lower
        ):
            contradictions += 1
...
def score_duplication(
    ...
    for ref in reference_data:
        ref_content = (
            f"{ref.get('human_statement', '')} {ref.get('agent_statement', '')}"
        )
...
def score_drift(
    payload: dict[str, Any], reference_vectors: dict | None = None
) -> float:
    """
...
def evaluate_reward(
    ...
    symbiosis = score_symbiosis(
        payload.get("human_statement", ""), payload.get("agent_statement", "")
    )
    ...
    duplication = penalties.get(
        "duplication", score_duplication(payload, reference_data)
    )
    ...
    gic = max(0.0, base * (integrity_score - P))
    ...
    return {
        ...
        "splits": {"human": human_share, "agent_pool": agent_share},
        "seal": seal,
        "timestamp": time.time(),
    }
