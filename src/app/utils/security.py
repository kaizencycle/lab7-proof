...
from ..crypto.ed25519 import canonical_json, ed25519_verify


def verify_gi_score(
    x_gi_score: str | None = Header(None), min_threshold: float = 0.95
) -> float:
    """
    ...
    try:
        ...
        if gi_score > 1.0:
            raise HTTPException(status_code=400, detail="GI score cannot exceed 1.0")
        return gi_score
    except ValueError as exc:
        raise HTTPException(
            status_code=400, detail="Invalid X-GI-Score format"
        ) from exc


def verify_signature_headers(
    x_sig: str | None = Header(None), x_key_id: str | None = Header(None)
) -> tuple[str, str]:
    """
    ...
    if not x_key_id.startswith("did:web:"):
        raise HTTPException(
            status_code=400, detail="X-Key-Id must be a DID in did:web: format"
        )
...
def verify_request_signature(
    request_body: dict, signature: str, key_id: str, public_key_b64: str
) -> bool:
    """
...
def validate_shield_headers(
    x_gi_score: str | None = Header(None),
    x_sig: str | None = Header(None),
...
def log_security_event(
    event_type: str, key_id: str, gi_score: float, success: bool, details: dict = None
) -> None:
    """
    ...
    print(
        f"Security Event: {event_type} - Key: {key_id} - GI: {gi_score} - Success: {success}"
    )
    if details:
