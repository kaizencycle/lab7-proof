...
class QualityMetricsService:
    ...
    def __init__(self):
        self.metrics_enabled = (
            os.getenv("QUALITY_METRICS_ENABLED", "false").lower() == "true"
        )
        ...
        for metric_name in quality_metrics:
            if isinstance(quality_metrics[metric_name], list):
                quality_metrics[metric_name] = [
                    record
                    for record in quality_metrics[metric_name]
    ...
    def _update_derived_metrics(self):
        ...
        outputs_with_provenance = sum(
            1 for o in quality_metrics["outputs"] if o["has_provenance"]
        )
        provenance_coverage = (
            outputs_with_provenance / total_outputs if total_outputs > 0 else 0
        )

        # Calculate hallucination rate (outputs without sources)
        outputs_without_sources = sum(
            1 for o in quality_metrics["outputs"] if not o["sources"]
        )
        hallucination_rate = (
            outputs_without_sources / total_outputs if total_outputs > 0 else 0
        )
    ...
    def get_current_metrics(self) -> dict:
        ...
        if quality_metrics["rollback_rate"]:
            recent_rollbacks = len(
                [
                    r
                    for r in quality_metrics["rollback_rate"]
            ...
            recent_outputs = len(
                [
                    o
                    for o in quality_metrics["outputs"]
                    if datetime.fromisoformat(o["timestamp"])
                    > datetime.utcnow() - timedelta(days=7)
                ]
            )
            rollback_rate = (
                recent_rollbacks / recent_outputs if recent_outputs > 0 else 0
            )
        else:
            rollback_rate = 0
        ...
        if quality_metrics["copilot_overlap_score"]:
            recent_scores = [
                c["overlap_score"]
                for c in quality_metrics["copilot_overlap_score"]
    ...
    def get_historical_metrics(self, hours: int = 24) -> dict:
        ...
        return {"status": "enabled", "period_hours": hours, "metrics": historical}
...
def record_rollback(reason: str, output_hash: str | None = None):
    """Record a rollback event."""
...
def quality_health():
    ...
    return {
        ...
        "total_records": sum(
            len(records)
            for records in quality_metrics.values()
