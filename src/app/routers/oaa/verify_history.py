...
try:
    ...
    raise RuntimeError("PyNaCl is required. Add 'pynacl' to requirements.txt") from exc
...
class VerifyKeyHistoryBody(BaseModel):
    ...
    payload: str | None = Field(
        default=None, description="Exact UTF-8 string that was signed (preferred)."
    )
    ...
    signature: str = Field(
        ..., description="Signature as 'ed25519:<base64>' or base64 only."
    )
...
def verify_key_history(body: VerifyKeyHistoryBody):
    ...
    if body.payload:
        ...
        try:
            ...
            raise HTTPException(
                status_code=400,
                detail="Bad digest format; need sha256 hex or 'sha256:<hex>'.",
            ) from exc
        signed_label = "digest"
    else:
        raise HTTPException(
            status_code=400, detail="Provide either 'payload' or 'digest'."
        )
