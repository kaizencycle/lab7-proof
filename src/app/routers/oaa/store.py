...
def list_sources(
    min_score: float = 0.0, gate: str | None = None
) -> list[tuple[Source, SourceScore]]:
    out = []
    for sid, s in SOURCES.items():
        sc = SCORES.get(sid)
        if not sc:
            continue
        if sc.composite < min_score:
            continue
        if gate and sc.policy_gate != gate:
            continue
        out.append((s, sc))
...
def summarize_reputation(source_id: str) -> float:
    ...
    stake_bonus = sum(
        v["stake_gic"] * (1 if v["opinion"] == "up" else -1) for v in votes
    )
