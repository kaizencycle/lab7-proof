...
def score_source(src: Source) -> SourceScore:
    ...
    if src.domain:
        prov += 0.3
    if src.owner and ("org" in src.owner or "contact" in src.owner):
        prov += 0.3
    if src.endpoints:
        prov += 0.2
    prov = _clamp(prov)
    ...
    perm = (
        1.0
        if lic in {"mit", "apache-2.0", "bsd-3-clause", "cc-by-4.0", "cc0"}
        else 0.7 if lic else 0.6
    )
