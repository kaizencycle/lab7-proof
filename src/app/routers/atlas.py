...
from ..crypto.ed25519 import canonical_json
from ..utils.capsule_verify import verify_capsule as verify_capsule_util
from ..utils.security import log_security_event, validate_shield_headers
...
def verify_capsule_simple(
    capsule: dict[str, Any], min_gi: float = GI_MIN
) -> dict[str, Any]:
    """
...
def audit():
    ...
    return {
        ...
        "capsules_latest": [
            capsule["id"] for capsule in CAPSULES_STORE[-5:]
        ],  # Last 5 capsules
    }
...
async def attest(
    ...
    return {"accepted": True, "ledger_ref": ledger_ref, "gi": gi_score}


@router.get("/capsules")
def get_capsules(limit: int = 10):
    """Returns the last N .gic capsule IDs + metadata"""
    return {"items": CAPSULES_STORE[-limit:] if CAPSULES_STORE else []}
...
def ingest_capsule(
    ...
    log_security_event(
        "capsule_ingest_success",
        key_id,
        gi_score,
        True,
        {"capsule_id": capsule.get("id")},
    )
    return {"ok": True, "capsule_id": capsule.get("id"), "stored": True}
